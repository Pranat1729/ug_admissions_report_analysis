...
# ══════════════════════════════════════════════
# LOAD CATEGORY LOOKUP FROM MONGO HIST DATA
# Freshmen: db["Freshmen"] has HS_Name + Recruitment_Category (after process_school_data)
# Transfers: db["Transfers"] has LAST_COL_UGRD_DESCR + Recruitment_Category
# We recompute categories from the raw hist collections so we don't duplicate logic.
# ══════════════════════════════════════════════
...
# ── Load & enrich with hist categories ────────
df_26 = pd.read_csv(uploaded)

# Build category lookup from MongoDB historical data
category_lookup = load_category_lookup(is_fresh)

# Map hist category onto 26 data; anything not in lookup → Unclassified
df_26["Hist_Category"] = df_26[school_col_26].map(category_lookup).fillna("Unclassified")

# If the file already has Recruitment_Category (from analysis script), keep it;
# otherwise use the one we just derived from hist data.
...
# ════════════════════════════════════════════════════════
# SECTION 2 — MATRICULATION BY CATEGORY
# ════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════
# SECTION 3 — ADMIT RATE vs YIELD RATE GAP
# ════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════
# SECTION 4 — PROGRAM-LEVEL EXPECTED ENROLLMENT
# ════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════
# SECTION 5 — SCHOOL-LEVEL DETAIL TABLE
# ════════════════════════════════════════════════════════
