...
def load_data(school_col, term_col):
    df_school = pd.DataFrame(
        list(db[school_col].find({}, {"_id": 0}))
    )
    df_term = pd.DataFrame(
        list(db[term_col].find({}, {"_id": 0}))
    )
...
# ----------------------------
# SCHOOL-LEVEL LOGIC (CACHED PROCESSING)
# ----------------------------
...
if not df_term.empty:
    ...
    semester_yield["new_yield"] = (
        semester_yield["current_yield"] * (1 + relative_increase)
    ).clip(upper=1)
    ...
    st.metric(
        "💰 Total Money Lost (historical max yield 60%)",
        f"${total_realistic_lost:,.0f}"
    )
...
if not df_term.empty:
    ...
    school_data = yearly[
        yearly[name_field]==selected_school
    ].sort_values("Year")
