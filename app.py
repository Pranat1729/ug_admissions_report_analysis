...
for key, val in list(field_map.items()):
    if val not in df.columns:
        st.warning(f"Column `{val}` not found; skipping mapping.")
        field_map.pop(key)
...
# Only compute additional revenue on existing aggregated data
semester_yield = df.groupby([field_map["name"], field_map["term"]]).agg(
...
# Merge into main df
df = df.merge(additional_revenue_hs, on=field_map["name"], how="left")
...
# DISPLAY
# ----------------------------
category = st.selectbox("Filter by Recruitment Category", ["All"] + list(df['Recruitment_Category'].unique()))
if category != "All":
...
yearly = df.groupby([field_map["name"], "Year"]).agg(
